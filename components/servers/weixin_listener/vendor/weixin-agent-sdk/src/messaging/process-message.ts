...
export async function processOneMessage(
  ...
  // --- Skip stale messages (older than 20 seconds) ---
  const MAX_MSG_AGE_MS = 20_000;
  if (full.create_time_ms) {
    const age = receivedAt - full.create_time_ms;
    if (age > MAX_MSG_AGE_MS) {
      deps.log(`[weixin] 跳过过期消息: age=${Math.round(age / 1000)}s, text="${textBody.slice(0, 30)}"`);
      return;
    }
  }
